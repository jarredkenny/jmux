...
export class LinearAdapter implements IssueTrackerAdapter {
  ...
  async getLinkedIssue(mrUrl: string): Promise<Issue | null> {
    const query = `query($url: String!) { attachments(filter: { url: { eq: $url } }, first: 1) { nodes { issue { id identifier title state { name } assignee { name } attachments { nodes { url } } url } } } }`;
  ...
  async getIssueByBranch(branch: string): Promise<Issue | null> {
    ...
    const query = `query($identifier: String!) { issueSearch(query: $identifier, first: 1) { nodes { id identifier title state { name } assignee { name } attachments { nodes { url } } url } } }`;
  ...
  async pollIssue(issueId: string): Promise<Issue> {
    const query = `query($id: String!) { issue(id: $id) { id identifier title state { name } assignee { name } attachments { nodes { url } } url } }`;
