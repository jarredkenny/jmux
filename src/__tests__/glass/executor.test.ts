...
describe("GlassExecutor", () => {
  ...
  test("restore: keeps the record if a restore command fails", () => {
    const calls: string[][] = [];
    const runner: GlassRunner = {
      run(args) {
        calls.push(args);
        if (args[0] === "list-windows") return { ok: true, lines: ["@5"] };
        if (args[0] === "list-sessions") return { ok: true, lines: ["$2"] };
        if (args[0] === "display-message") return { ok: true, lines: ["@88"] };
        if (args[0] === "join-pane") return { ok: false, lines: [] }; // fails
    ...
    // record retained for retry; select-layout never attempted after join-pane failed
    expect(store.map.has("%7")).toBe(true);
    expect(store.events).not.toContain("remove:%7");
